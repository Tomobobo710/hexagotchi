...
bool DialogBox::shouldAdvance() const {
    return isFinished() && (autoAdvancePending || (onFinished && !hasOptions()));
}
...
void DialogBox::skip() {
    skipCharacterReveal();
    // If auto-continue is enabled, also skip the auto-continue timer
    if (autoContinueEnabled) {
        autoContinueTimer = autoContinueDuration;
        autoAdvancePending = true;
    }
    // Trigger the finished callback immediately if text is now complete
